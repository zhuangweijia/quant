...
def upgrade() -> None:
    ...
    op.add_column("equity_snapshots", sa.Column("timestamp", sa.DateTime(timezone=True), nullable=True))
    op.execute(
        "UPDATE equity_snapshots SET timestamp = (date || ' 15:30:00+00')::timestamptz"
    )
    op.alter_column("equity_snapshots", "timestamp", nullable=False)
    op.drop_column("equity_snapshots", "date")
    op.create_unique_constraint("uq_equity_snapshot_user_timestamp", "equity_snapshots", ["user_id", "timestamp"])
...
def downgrade() -> None:
    ...
    op.create_unique_constraint("uq_equity_snapshot_user_date", "equity_snapshots", ["user_id", "date"])
