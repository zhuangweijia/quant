...
async def trigger_analysis(user: CurrentUser, db: DBSession):
    ...
    if running:
        return ResponseBase(data=AnalysisTriggerResponse(run_id=running.run_id, status="running"))
...
async def get_analysis_status(db: DBSession):
    result = await db.execute(select(AnalysisRun).order_by(AnalysisRun.started_at.desc()).limit(1))
