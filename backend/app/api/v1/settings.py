...
async def save_notifications(user: CurrentUser, db: DBSession, payload: NotificationConfigRequest):
    ...
    if payload.email_password:
        await set_setting(
            db, user.id, "notification", "email_password", payload.email_password, encrypted=True
        )
    if payload.webhook_secret:
        await set_setting(
            db, user.id, "notification", "webhook_secret", payload.webhook_secret, encrypted=True
        )
...
async def test_email(user: CurrentUser, db: DBSession):
    ...
    success = await send_email(
        db, user.id, "StockAnalysis 测试邮件", "<h2>测试成功</h2><p>邮件通知配置正常。</p>"
    )
...
async def change_password(user: CurrentUser, db: DBSession, payload: PasswordChangeRequest):
    ...
    if not db_user or not AuthService.verify_password(
        payload.old_password, db_user.hashed_password
    ):
